...
ll solve(){
  // formuala : Matrix operations
  // An = An-1 * A, A2n = An * An
  // Sn = An + An-1 + ... + A1 = An + Sn-1
  // S2n = A2n + ... + A1 = An*(An + ... + A1) + (An + ... + A1)
  //     = (An + 1)*Sn
  ll res = 0;
  Matrix m;
  getS(m, K - 1);

  // Matrix M[i][j] : # of path i -> j
  // TODO : verify
  for (int i =0 ; i< N; i++)
    res = (res + m.arr[i][i]) % M;
  return res;
...
// init the values of (A^2)^k v
void init() {
  int k = 0;
  Matrix A;
  Matrix m_result;

  // init the original matrix
  copy(mat_ori, A);
  copy(A, memo[0]);

  // get the 2 powered A matrices
  int n  = K - 1;
  while(n > 1){
    k++; n >>= 1;
    product(A, A, m_result);
    copy(m_result, A);
    copy(A, memo[k]);
  }
  return;
