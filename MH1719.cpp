...
int solve(){
  int result = 0;
  int temp[N_MAX] = {0,};
  // 01. sort the weight array
  _quickSort (weight, 0, N-1);

  // 02. scanf the top array to make a maximum box
  int i_bottom = 0;
  for (int i_top=N-1; i_top >= 0; i_top--){

    // 03. find the appropriate # of items for a box
    int cnt_item = 1;
    for (int div=1; ; div++){
      if (weight[i_top] * div >= 50){
        i_bottom += div - 1;

        // in case of bottom items are empty
        if (i_bottom > i_top)
          break;
